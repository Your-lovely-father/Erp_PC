...
<script>
    ...
    export default {
        ...
        methods: {
            onPage() {
                this.$store.commit('showTracking', true);
                this.$store.commit('showStatusTracking', false);
            },
            cancel(){
                this.onPage()
            },
            confirm(){
                this.onPage();
                Api.trackingAdd(this.guide_look_content,this.client_id,this.user_id,this.area_id,this.storefront_id,this.look_time).then((res)=>{
                    if(res.code==="200001"){
        ...
        computed: {
            addTracking() {
                return this.$store.state.tracking.addTracking
            },
        },
    }
</script>
